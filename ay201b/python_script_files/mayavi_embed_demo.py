...
class TDViz(HasTraits):
	...
	view = View(
      HSplit(
		VGroup(
			...
			Item("save_the_scene", tooltip=u"Save current scene in a .obj file. There is no guarantee that everything you see are saved as they are!", visible_when="rendering=='Surface'"),
	...
	def _fitsfile_changed(self):
		...
		dat = img[0].data
		#self.data = dat[0]
		self.hdr  = img[0].header			 # Header not to be used in this code, but might be useful...
