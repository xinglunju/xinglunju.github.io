...
def __gauss_tau__(axis,p):
	...
	phijk = 1/sqrt(2 * pi) / (sigma * 1e9) * np.exp(-1.0 * (axis - fsky)**2 / sigma**2)
...
def __model_11__(params, faxis, spec):
	...
	For now we assume seven components (K=0-6).
	Then subtract data.
...
def fit_spec(spec, faxis, Jupp=12, K_fit=7, cutoff=0.009, varyf=2, interactive=True, mode='single'):
	...
	while unsatisfied:
		...
		if vlsr1 != 0:
			...
			params.add('sigma', value=0.003, min=0, max=0.050)
			if varyf > 0:
		...
		if interactive:
			...
			if vlsr1 != 0 and vlsr2 != 0:
				...
				plt.text(0.02, 0.90, r'V$_{lsr}$=%.1f km/s' % vlsr1, transform=ax.transAxes, color='r', fontsize=15)
				plt.text(0.02, 0.85, r'T$_{rot}$=%.1f($\pm$%.1f) K' % (params['T'].value,params['T'].stderr), transform=ax.transAxes, color='r', fontsize=15)
				plt.text(0.02, 0.80, r'N$_{tot}$=%.2e($\pm$%.2e) cm$^{-2}$' % (params['Ntot'].value,params['Ntot'].stderr), transform=ax.transAxes, color='r', fontsize=15)
				plt.text(0.02, 0.75, r'FWHM=%.2f($\pm$%.2f) km/s' % (c*params['sigma'].value/ch3cn_info['frest'][0]/1e5*2.355,c*params['sigma'].stderr/ch3cn_info['frest'][0]/1e5*2.355), transform=ax.transAxes, color='r', fontsize=15)
			plt.legend()
			plt.show()
