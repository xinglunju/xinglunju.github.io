...
def fit_2comp(data_low, data_upp, ra_range=[0,1], dec_range=[0,1], cutoff=0.010, varyv=2, writefits=False, interactive=False):
	"""
	...
	for i in dec_range:
		for j in ra_range:
			...
			if vlsr1 != 0:
				...
				if varyv > 0:
					params.add('peakv', value=vlsr1, min=vlsr1-variv*onevpix, max=vlsr1+variv*onevpix)
				elif varyv == 0:
			...
			if vlsr2 != 0:
				...
				params.add('peakv_c2', value=vlsr2, min=vlsr2-variv*onevpix, max=vlsr2+variv*onevpix)
				params.add('sigmav_c2', value=1.0, min=0, max=5.0)
				params.add('peaki_s1_c2', expr="peaki*(1-exp(-tau11_s1_c2))/(1-exp(-tau11_c2))")
			...
			if interactive:
				...
				if vlsr1 != 0 and vlsr2 != 0:
					...
					pylab.text(0.05, 0.9, '1sr Trot=%.1f'%params['Trot'].value, transform=ax.transAxes, color='m')
					pylab.text(0.05, 0.8, '2nd Trot=%.1f'%params['Trot_c2'].value, transform=ax.transAxes, color='c')
				elif vlsr1 != 0 or vlsr2 != 0:
					pylab.text(0.05, 0.9, 'Trot=%.1f'%params['Trot'].value, transform=ax.transAxes, color='r')
				pylab.legend()
				pylab.show()
...
#fit_2comp(data1,data2,ra_range=[153,154],dec_range=[148,149],cutoff=0.006,varyv=0.0,writefits=False,interactive=True)
fit_2comp(data1,data2,ra_range=np.arange(naxisx),dec_range=np.arange(naxisy),cutoff=0.008,varyv=0,writefits=True,interactive=False)
